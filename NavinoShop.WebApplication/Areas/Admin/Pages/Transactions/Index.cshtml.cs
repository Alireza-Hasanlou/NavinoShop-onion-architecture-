...
namespace NavinoShop.WebApplication.Areas.Admin.Pages.Transactions
{
    public class IndexModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGet(TransactionFor transactionFor,TransactionSource transactionSource,
            TransactionStatus transactionStatus,TransactionType transactionType, int take = 10, int pageId = 1, string filter = "")
            
        {

            TransactionsModel = await _adminTransactionQueryService.GetTransactionsForAdmin(take,pageId,filter,transactionFor,transactionSource,transactionStatus,transactionType);
            return Page();
        }
    }
}
